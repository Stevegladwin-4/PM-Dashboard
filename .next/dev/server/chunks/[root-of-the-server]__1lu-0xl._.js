...
async function PATCH(request, { params }) {
    ...
    if ("completed" in body) {
        updateData.completed_date = body.completed ? new Date().toISOString().slice(0, 10) : null;
    ...
    const { data, error } = await supabase.from("pm_schedules").update(updateData).eq("id", id).select().single();
